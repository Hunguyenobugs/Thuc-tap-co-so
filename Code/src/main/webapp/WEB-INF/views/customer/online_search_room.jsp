    ...
    <%@ taglib prefix="c" uri="jakarta.tags.core" %>
        <%@ taglib prefix="fmt" uri="jakarta.tags.fmt" %>
            ...
            <head>
                ...
                <title>Đặt phòng online - Grand Lotus Hotel</title>
                ...
                <section class="section fade-in">
                    <div class="topbar" style="margin-bottom:24px;">
                        <div>
                            ...
                            <div class="breadcrumb"><span>Chọn phòng</span></div>
                        </div>
                    </div>
                    ...
                    <%-- Thông báo lỗi --%>
                        <c:if test="${param.error == 'invalid_dates' || not empty error}">
                            <div class="alert alert-danger"
                                style="background:#ff4d6d22;border:1.5px solid #ff4d6d;color:#ff4d6d;padding:12px 18px;border-radius:8px;margin-bottom:16px;">
                                <strong>
                                    <c:choose>
                        ...
                        <%-- Banner loại phòng đang chọn (hiện khi đã tìm kiếm) --%>
                            <c:if test="${selectedRoomType != null}">
                                <div class="card mb-3"
                                    style="border:2px solid var(--accent);background:linear-gradient(135deg, var(--bg-secondary), var(--bg-primary));">
                                    <div style="display:flex;align-items:center;gap:16px;flex-wrap:wrap;">
                                        <c:if test="${not empty selectedRoomType.imageUrl}">
                                            <c:set var="firstImg" value="${selectedRoomType.imageUrl.split(',')[0]}" />
                                            <img src="${pageContext.request.contextPath}${firstImg}"
                                                style="width:120px;height:80px;object-fit:cover;border-radius:8px;"
                                                alt="${selectedRoomType.name}">
                                        </c:if>
                                        <div style="flex:1;min-width:200px;">
                                            <h3 style="margin:0 0 4px 0;">${selectedRoomType.name}</h3>
                                            <div class="fs-sm text-muted">${selectedRoomType.capacity} khách •
                                                ${selectedRoomType.area}</div>
                                        </div>
                                        <div style="text-align:right;">
                                            <div class="text-accent fw-bold" style="font-size:20px;">
                                                <fmt:formatNumber value="${selectedRoomType.basePrice}"
                                                    pattern="#,##0" />₫<span class="fs-sm text-muted"
                                                    style="font-weight:400;"> /đêm</span>
                                            </div>
                                            ...
                                            <div class="form-row">
                                                <div class="form-group mb-0">
                                                    ...
                                                    <input type="date" id="checkInInput" name="checkIn"
                                                        class="form-control" value="${checkIn}" required>
                                                </div>
                                                <div class="form-group mb-0">
                                                    <label>Ngày trả phòng</label>
                                                    <input type="date" id="checkOutInput" name="checkOut"
                                                        class="form-control" value="${checkOut}" required>
                                                </div>
                                            </div>
                                            <div class="form-group mt-2">
                                                ...
                                                <select name="roomTypeId" class="form-control" required>
                                                    ...
                                                    <c:forEach var="rt" items="${roomTypes}">
                                                        <option value="${rt.id}"
                                                            ${selectedType==rt.id.toString()?'selected':''}>${rt.name} -
                                                            <fmt:formatNumber value="${rt.basePrice}" pattern="#,##0" />
                                                            ₫/đêm (${rt.capacity} khách)
                                                        </option>
                                                    </c:forEach>
                                                </select>
                                            </div>
                                        ...
                                        <div class="card">
                                            <div class="card-header">
                                                ...
                                                <c:if test="${not empty rooms}">
                                                    <span class="badge badge-success">${rooms.size()} phòng khả
                                                        dụng</span>
                                                </c:if>
                                            </div>
                                            <c:choose>
                                                <c:when test="${empty rooms}">
                                                    <div class="no-data">Không có phòng trống trong khoảng thời gian
                                                        này.<br><small>Hãy thử chọn ngày khác hoặc loại phòng
                                                            khác.</small></div>
                                                </c:when>
                                                <c:otherwise>
                                                                ...
                                                                <c:forEach var="r" items="${rooms}">
                                                                    <tr>
                                                                        ...
                                                                        <td class="text-accent fw-bold">
                                                                            <fmt:parseNumber value="${r.basePrice}"
                                                                                var="rp" integerOnly="false" />
                                                                            <fmt:formatNumber value="${rp}"
                                                                                pattern="#,##0" />₫
                                                                        </td>
                                                                        <td>
                                                                            <form method="post"
                                                                                action="${pageContext.request.contextPath}/onlineBooking"
                                                                                ...
                                                                                <input type="hidden" name="roomTypeId"
                                                                                    value="${selectedType}">
                                                                                <button type="submit"
                                                                                    class="btn btn-success btn-sm">Chọn
                                                                                    phòng này</button>
                                                                            </form>
                                                                        </td>
                                                                    </tr>
                                    ...
                                    <div class="card" style="position:sticky;top:100px;">
                                        ...
                                        <c:choose>
                                            <c:when test="${empty cart}">
                                                <div class="no-data" style="padding:20px;">Chưa có phòng
                                                    nào<br><small>Tìm và thêm phòng vào giỏ bên trái</small></div>
                                            </c:when>
                                            <c:otherwise>
                                                    ...
                                                    <div class="card mb-2"
                                                        ...
                                                        <div class="flex-between">
                                                            <div>
                                                                ...
                                                                <div class="fs-sm text-muted">
                                                                    <fmt:parseDate value="${item.checkIn}"
                                                                        pattern="yyyy-MM-dd" var="pCI2" />
                                                                    <fmt:formatDate value="${pCI2}" pattern="dd/MM" /> →
                                                                    <fmt:parseDate value="${item.checkOut}"
                                                                        pattern="yyyy-MM-dd" var="pCO2" />
                                                                    <fmt:formatDate value="${pCO2}"
                                                                        pattern="dd/MM/yyyy" />
                                                                    (${item.nights} đêm)
                                                                </div>
                                                            </div>
                                                ...
                                                <!-- Tổng -->
                                                <div
                                                    style="border-top:2px solid var(--accent);padding-top:12px;margin-top:8px;text-align:center;">
                                                ...
                                                <a href="${pageContext.request.contextPath}/onlineBooking?action=confirm"
                                        ...
                                        <a href="${pageContext.request.contextPath}/search"
                                            class="btn btn-outline btn-block mt-2">← Quay lại tìm phòng</a>
                                    </div>
                                </div>
                            </div>
