...
<body><div class="layout manager-layout"><jsp:include page="../components/sidebar.jsp"/>`n    <jsp:include page="../components/manager_header.jsp"/><main class="main-content fade-in">
    ...
    <% if ("has_booking".equals(request.getParameter("error"))) { %><div class="alert alert-danger">Không thể xóa: phòng đang có phiếu đặt</div><% } %>
            ...
            <c:otherwise><div class="table-container"><table>
                ...
                <tbody><c:forEach var="r" items="${results}">
                    <tr><td><strong>${r.roomNumber}</strong></td><td>${r.floor}</td><td>${r.roomTypeName}</td>
                        ...
                        <td><div class="btn-group"><a href="${pageContext.request.contextPath}/manager/room?action=edit&id=${r.id}" class="btn btn-outline btn-sm">Sửa</a>
                            <button onclick="confirmDelete('${pageContext.request.contextPath}/manager/room?action=delete&id=${r.id}','Phòng ${r.roomNumber}')" class="btn btn-danger btn-sm">Xóa</button></div></td></tr>
