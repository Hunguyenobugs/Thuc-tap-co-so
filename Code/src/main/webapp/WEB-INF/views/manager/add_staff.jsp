...
<body><div class="layout manager-layout"><jsp:include page="../components/sidebar.jsp"/>
    ...
    <div class="card" style="max-width:600px;"><form method="post" action="${pageContext.request.contextPath}/manager/staff"><input type="hidden" name="action" value="insert">
        <div class="form-row">
            ...
            <div class="form-group"><label>Mã NV <span class="required">*</span></label><input type="text" name="employeeCode" class="form-control" placeholder="VD: NV007" value="${param.employeeCode}" required></div>
        </div>
        <div class="form-row">
            <div class="form-group"><label>Username <span class="required">*</span></label><input type="text" name="username" class="form-control" value="${param.username}" required></div>
            <div class="form-group"><label>Vai trò</label><select name="role" class="form-control"><option value="STAFF" ${param.role == 'STAFF' ? 'selected' : ''}>Nhân viên</option><option value="MANAGER" ${param.role == 'MANAGER' ? 'selected' : ''}>Quản lý</option></select></div>
        </div>
        <div class="form-row">
            <div class="form-group"><label>Email <span class="required">*</span></label><input type="email" name="email" class="form-control" value="${param.email}" required></div>
            <div class="form-group"><label>SĐT <span class="required">*</span></label><input type="tel" name="phone" class="form-control" value="${param.phone}" required></div>
        </div>
        <div class="form-row">
