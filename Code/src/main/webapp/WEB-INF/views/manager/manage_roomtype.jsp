...
<main class="main-content fade-in">
    <div class="topbar"><div><h1>Quản lý <span>loại phòng</span></h1></div></div>
    <c:if test="${param.msg != null}"><div class="alert alert-success">Thao tác thành công!</div></c:if>
    <c:if test="${param.error == 'has_rooms'}"><div class="alert alert-danger">Không thể xóa: loại phòng này đang có phòng liên kết</div></c:if>
                ...
                <c:forEach var="rt" items="${list}">
                    <tr>
                        ...
                        <td><div class="btn-group">
                            <a href="${pageContext.request.contextPath}/manager/roomtype?action=edit&id=${rt.id}" class="btn btn-outline btn-sm">Sửa</a>
                            <button onclick="confirmDelete('${pageContext.request.contextPath}/manager/roomtype?action=delete&id=${rt.id}','${rt.name}')" class="btn btn-danger btn-sm">Xóa</button>
                        </div></td>
                    </tr>
                </c:forEach>
                </tbody>
