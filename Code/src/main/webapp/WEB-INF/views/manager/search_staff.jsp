...
<body><div class="layout manager-layout"><jsp:include page="../components/sidebar.jsp"/>`n    <jsp:include page="../components/manager_header.jsp"/><main class="main-content fade-in">
    ...
    <c:if test="${results != null}"><c:choose><c:when test="${empty results}"><div class="no-data">Không tìm thấy</div></c:when><c:otherwise>
        ...
        <c:forEach var="u" items="${results}"><tr><td>${u.employeeCode}</td><td><strong>${u.fullName}</strong></td><td>${u.username}</td>
            ...
            <td><div class="btn-group"><a href="${pageContext.request.contextPath}/manager/staff?action=edit&id=${u.id}" class="btn btn-outline btn-sm">✏️</a>
                <button onclick="confirmDelete('${pageContext.request.contextPath}/manager/staff?action=delete&id=${u.id}','${u.fullName}')" class="btn btn-danger btn-sm">🗑️</button></div></td></tr></c:forEach>
