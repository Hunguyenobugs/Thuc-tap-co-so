...
<body><div class="layout manager-layout"><jsp:include page="../components/sidebar.jsp"/>`n    <jsp:include page="../components/manager_header.jsp"/><main class="main-content fade-in">
    ...
    <% if ("in_use".equals(request.getParameter("error"))) { %><div class="alert alert-danger">Không thể xóa: dịch vụ đã được sử dụng trong phiếu đặt</div><% } %>
    ...
    <c:if test="${results != null}"><c:choose><c:when test="${empty results}"><div class="no-data">Không tìm thấy</div></c:when><c:otherwise>
        ...
        <c:forEach var="s" items="${results}"><tr><td><strong>${s.name}</strong></td><td><span class="badge badge-info">${s.category}</span></td><td>${s.unit}</td><td class="text-accent fw-bold"><fmt:formatNumber value="${s.price}" pattern="#,##0"/>₫</td>
            <td><div class="btn-group"><a href="${pageContext.request.contextPath}/manager/service?action=edit&id=${s.id}" class="btn btn-outline btn-sm">Sửa</a>
                <button onclick="confirmDelete('${pageContext.request.contextPath}/manager/service?action=delete&id=${s.id}','${s.name}')" class="btn btn-danger btn-sm">Xóa</button></div></td></tr></c:forEach>
