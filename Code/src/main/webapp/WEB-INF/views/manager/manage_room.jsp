...
<main class="main-content fade-in">
    ...
    <c:if test="${param.error == 'has_booking'}"><div class="alert alert-danger">Không thể xóa: phòng đang có phiếu đặt</div></c:if>
                ...
                <c:forEach var="r" items="${list}">
                    <tr>
                        ...
                        <td><div class="btn-group">
                            <a href="${pageContext.request.contextPath}/manager/room?action=edit&id=${r.id}" class="btn btn-outline btn-sm">Sửa</a>
                            <button onclick="confirmDelete('${pageContext.request.contextPath}/manager/room?action=delete&id=${r.id}','Phòng ${r.roomNumber}')" class="btn btn-danger btn-sm">Xóa</button>
                        </div></td>
                    </tr>
                </c:forEach>
                </tbody>
