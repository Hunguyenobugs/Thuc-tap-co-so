...
<main class="main-content fade-in">
    <div class="topbar"><div><h1>Quản lý <span>dịch vụ</span></h1></div></div>
    <c:if test="${param.msg != null}"><div class="alert alert-success">Thao tác thành công!</div></c:if>
    <c:if test="${param.error == 'in_use'}"><div class="alert alert-danger">Không thể xóa: dịch vụ đã được sử dụng trong phiếu đặt</div></c:if>
                ...
                <c:forEach var="s" items="${list}">
                    <tr>
                        ...
                        <td><div class="btn-group">
                            <a href="${pageContext.request.contextPath}/manager/service?action=edit&id=${s.id}" class="btn btn-outline btn-sm">Sửa</a>
                            <button onclick="confirmDelete('${pageContext.request.contextPath}/manager/service?action=delete&id=${s.id}','${s.name}')" class="btn btn-danger btn-sm">Xóa</button>
                        </div></td>
                    </tr>
                </c:forEach>
                </tbody>
