        ...
        <div>
            <div class="card mb-3"><div class="card-header"><h3>Dịch vụ đã sử dụng</h3><span class="badge badge-info">Tổng: <fmt:formatNumber value="${serviceTotal}" pattern="#,##0"/>₫</span></div>
                <c:choose><c:when test="${empty usedServices}"><div class="no-data">Chưa có dịch vụ nào</div></c:when><c:otherwise>
                    ...
                    <c:forEach var="us" items="${usedServices}"><tr><td>${us.serviceName}</td><td>${us.quantity} ${us.serviceUnit}</td><td><fmt:formatNumber value="${us.unitPrice}" pattern="#,##0"/>₫</td><td class="fw-bold"><fmt:formatNumber value="${us.subtotal}" pattern="#,##0"/>₫</td><td class="fs-sm"><fmt:formatDate value="${us.usedDate}" pattern="HH:mm dd/MM/yyyy"/></td></tr></c:forEach>
