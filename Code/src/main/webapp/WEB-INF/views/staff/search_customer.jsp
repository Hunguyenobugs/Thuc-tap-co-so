...
<body><div class="layout staff-layout"><jsp:include page="../components/sidebar.jsp"/>
    ...
    <div class="topbar"><div><h1>📝 Đặt phòng - <span>Tìm khách hàng</span></h1><div class="breadcrumb"><span>Bước 1: Chọn khách hàng</span></div></div></div>
    <form class="search-bar" method="get" action="${pageContext.request.contextPath}/staff/booking">
        ...
        <input type="text" name="q" class="form-control" placeholder="Nhập số CCCD hoặc tên khách hàng..." value="${keyword}">
        <button type="submit" class="btn btn-primary">🔍 Tìm</button>
        <a href="${pageContext.request.contextPath}/staff/booking?action=addCustomer" class="btn btn-success">➕ Khách mới</a>
    </form>
    <c:if test="${customers != null}">
        <c:choose>
            <c:when test="${empty customers}"><div class="no-data">Không tìm thấy khách hàng. <a href="${pageContext.request.contextPath}/staff/booking?action=addCustomer">Thêm khách mới</a></div></c:when>
            <c:otherwise>
                <div class="table-container"><table><thead><tr><th>CCCD</th><th>Họ tên</th><th>SĐT</th><th>Email</th><th></th></tr></thead><tbody>
                    <c:forEach var="c" items="${customers}"><tr>
                        <td>${c.idCard}</td><td><strong>${c.fullName}</strong></td><td>${c.phone}</td><td>${c.email}</td>
                        <td><a href="${pageContext.request.contextPath}/staff/booking?action=searchRoom&customerId=${c.id}" class="btn btn-primary btn-sm">Chọn →</a></td>
                    </tr></c:forEach>
                </tbody></table></div>
            </c:otherwise>
        </c:choose>
