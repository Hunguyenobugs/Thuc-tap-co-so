                    ...
                    <main class="main-content fade-in">
                        ...
                        <div class="topbar">
                            <div>
                                ...
                                <div class="breadcrumb"><span>Bước 2: Tìm và chọn phòng</span></div>
                            </div>
                            <div>
                                <span class="badge badge-primary" style="font-size:14px;padding:8px 14px;">Khách:
                                        ...
                                        <div class="card">
                                            ...
                                            <c:choose>
                                                <c:when test="${empty rooms}">
                                                    <div class="no-data">Không có phòng trống trong khoảng thời gian này
                                                    </div>
                                                </c:when>
                                                <c:otherwise>
                                                            ...
                                                            <tbody>
                                                                <c:forEach var="r" items="${rooms}">
                                                                    <tr>
                                                                        ...
                                                                        <td>
                                                                            <form method="post"
                                                                                action="${pageContext.request.contextPath}/staff/booking"
                                                                                ...
                                                                                <input type="hidden" name="customerId"
                                                                                    value="${customer.id}">
                                                                                <button type="submit"
                                                                                    class="btn btn-success btn-sm">Chọn</button>
                                                                            </form>
                                                                        </td>
                                                                    </tr>
                                                                </c:forEach>
                                        ...
                                        <c:choose>
                                            <c:when test="${empty cart}">
                                                <div class="no-data" style="padding:20px;">Chưa có phòng
                                                    nào<br><small>Tìm và thêm phòng vào giỏ</small></div>
                                            </c:when>
                                            <c:otherwise>
                                                            ...
                                                            <div>
                                                                ...
                                                                <div class="fs-sm text-muted">
                                                                    <fmt:parseDate value="${item.checkIn}"
                                                                        pattern="yyyy-MM-dd" var="pCI" />
                                                                    <fmt:formatDate value="${pCI}" pattern="dd/MM" /> →
                                                                    <fmt:parseDate value="${item.checkOut}"
                                                                        pattern="yyyy-MM-dd" var="pCO" />
                                                                    <fmt:formatDate value="${pCO}"
                                                                        pattern="dd/MM/yyyy" />
                                                                    (${item.nights} đêm)
                                                                </div>
                                                            </div>
                                                ...
                                                <a href="${pageContext.request.contextPath}/staff/booking?action=confirm"
                                                    class="btn btn-primary btn-block btn-lg mt-3">
                                                    Xác nhận đặt (${cart.size()} phòng)
                                                </a>
                                            </c:otherwise>
                                        </c:choose>
                                        <a href="${pageContext.request.contextPath}/staff/booking?action=searchCustomer&reset=true"
                                            class="btn btn-outline btn-block mt-2"> Chọn lại khách</a>
                                    </div>
                                </div>
                            </div>
